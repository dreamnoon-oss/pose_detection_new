...
| `min_arm_torso_angle` | 0 (不检查) | 手臂与躯干最小夹角 |
...
| 躯干夹角下限 | 45° | parallel_line | 臂与躯干夹角下限，防未抬臂 |
| 指向角度阈值 | 30° | pointing | 臂方向与区域夹角上限 |
| 宽松指向阈值 | 55° | pointing_with_line | 组合检测的指向条件 |
| 延长倍数 | 6× | pass_region | 射线延长倍率 |
| 最小臂长 | 30px | 全部 | 过滤无效短臂 |
| 持续帧数 | 15 | 全部 | 连续命中确认事件 |
| 帧衰减 | -2/帧 | 全部 | 容忍短暂丢帧 |
| 冷却期 | 45帧 | 全部 | 事件触发后同规则暂停 |
