...
def generate_report(output_path, *,
    ...
    Args:
        ...
        stops: List of per-stop dicts, each with keys:
            arrive/depart/duration (str or None) and
            action_results (list from SequenceAnalyzer).
            One block per train stop; a single-stop video has one entry.
    ...
    if not stops:
        w.writerow(["检测结果"])
        w.writerow(["状态", "未检测到列车到站，无动作检测结果"])
        w.writerow([])
    ...
    for i, stop in enumerate(stops, 1):
        ...
        w.writerow([])
        _write_stop_actions(w, stop.get("action_results") or [], action_mapping)
        w.writerow([])
...
def _write_stop_actions(w, action_results, action_mapping):
    """Write the action-result table + evaluation rows for one train stop."""
    w.writerow(["动作检测结果"])
    ...
    for r in action_results:
        ...
        if r.get('found'):
            prev_frame = r['frame']
    w.writerow(["顺序合规", "顺序正确" if order_ok else "顺序不正确"])

    all_found = total_found == total_expected
    w.writerow(["动作是否符合规范", "是" if (all_found and order_ok) else "否"])
